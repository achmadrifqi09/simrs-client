...
const rePrintValidation = z.object({
    identifier_type: z.string().min(1, { message: 'Jenis nomor harus diisi' }),
    identifier_code: z
        .string()
...
const nameValidation = z
    ...
    .refine(
        ...
        {
            message:
                'Nama tidak valid, harus terdiri dari minimal 3 karakter, tanpa karakter berulang atau pola mencurigakan.',
        }
    );

const patientValidationSchema = z.object({
    nama_pasien: nameValidation,
    no_hp: z.string().refine(
        (phoneNumber) => {
        ...
        {
            message:
                'Nomor HP tidak valid, harus dimulai dengan +62, 62, atau 08 dan diikuti dengan 10 hingga 12 digit angka.',
        }
    ),
    tgl_lahir: dateValidation,
});

const oldPatientValidationSchema = z
    .object({
    ...
    .superRefine((data, ctx) => {
        ...
        if (Number(data.patient_type) === 2 && !allowedLength.includes(identifierNumberLength)) {
            ctx.addIssue({
        ...
        if (Number(data.patient_type) === 4 && !allowedLength.includes(identifierNumberLength)) {
            ctx.addIssue({
                path: ['identifier_number'],
