...
const dateValidation = z.string().transform((val) => {
...
const patientValidation = z
    .object({
        ...
        tempat_lahir: z.string().max(100, 'Tempat Lahir tidak boleh melebihi 100 karakter'),
        ...
        identitas_pasien: z
            .string({ message: 'Identitas pasien harus berupa tesk' })
            .min(1, { message: 'Identitas pasien harus diisi' })
            .transform((value) => {
                return Number(value);
            }),
        no_identitas: z.string().max(100, 'No Identitas tidak boleh melebihi 100 karakter'),
        no_bpjs: z.string().max(50, 'No BPJS tidak boleh melebihi 50 karakter').nullish(),
        no_hp: z.string().regex(phoneNumberRegex, 'No HP harus berupa nomor telepon yang valid').max(20),
        ...
        rt_asal: z.string({ message: 'RT asal harus diisi' }).max(5, 'RT Asal tidak boleh melebihi 5 karakter'),
        rw_asal: z.string({ message: 'RW Asal harus diisi' }).max(5, 'RW Asal tidak boleh melebihi 5 karakter'),
        ...
        nama_pekerjaan: z.string().max(100, { message: 'Harus diisi maximal 100' }).nullish(),
    })
    .refine(
        (value) => {
...
const isSeventeenYearsOld = (birthDate: string) => {
    ...
    return age > 17 && currentDate.isSameOrAfter(birthDateMoment.clone().add(17, 'years'));
};
...
export { patientValidation, cancelValidation };
