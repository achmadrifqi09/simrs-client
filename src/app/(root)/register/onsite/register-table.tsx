...
const RegisterTable = ({
    ...
    const handleFilter = () => {
        ...
        if (patientType) {
            baseUrl = baseUrl.includes('?') ? `${baseUrl}&patient_type=${patientType}`
                : `${baseUrl}?patient_type=${patientType}`
        }

        if (guarantorType) {
            baseUrl = baseUrl.includes('?') ? `${baseUrl}&guarantor_type=${guarantorType}`
        ...
        <div className="w-full">
            ...
            <div>
                <div className="flex w-full justify-between mb-4 gap-4">
                    <Input type="text" id="fieldSearch" name="fieldSearch" className="w-full md:w-1/3"
                           placeholder="Cari data ..." onChange={debouncedChangeSearch}/>
                    <Popover open={showFilter} onOpenChange={setShowFilter}>
                        ...
                        <PopoverContent align="end" className="w-[20em] space-y-3">
                            <div>
                                ...
                                />
                                <Select onValueChange={(value) => setGuarantorType(Number(value))}
                                ...
                                <Select onValueChange={(value) => setPatientType((Number(value)))}
                ...
                <Table>
                    ...
                    <TableBody>
                        {(data && data?.results?.length === 0 && !loading) && (
                            <TableRow>
                                <TableCell colSpan={11} className="text-center">Data tidak ditemukan</TableCell>
                            </TableRow>
                        )}
                        {
                                ...
                                return (
                                    <React.Fragment key={index}>
                                        <TableRow>
                                            ...
                                            <TableCell>{Number(register.antrian?.jenis_penjamin === 2) ? 'BPJS' : 'Umum'}</TableCell>
                                            <TableCell>{Number(register.antrian?.jenis_pasien === 1) ? 'Lama' : 'Baru'}</TableCell>
                                            <TableCell>{Number(register.status_batal === 1) ? 'Batal' : 'Tidak'}</TableCell>
                                            <TableCell>
                                                <div className="flex gap-2">
                                                    {
                                                        ...
                                                        >
                                                            <Link
                                                                href={
                                                                    Number(register.antrian?.jenis_pasien === 1)
