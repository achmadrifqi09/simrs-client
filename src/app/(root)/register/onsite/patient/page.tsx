...
const PatientRegister = () => {
    ...
    const onSubmit = handleSubmit(async (values) => {
        try {
            ...
            if (response?.data) {
                ...
                router.push(
                    RID && response?.data?.kode_rm
                        ? `/register/onsite/${RID}?rm=${response?.data?.kode_rm}`
                        : '/register/onsite'
                );
                patientForm.reset({});
            }
    ...
    return (
        <div className="relative">
            <Heading variant="page-title" headingLevel="h3" className="mb-4">
                Update Data Pasien Pendaftaran
            </Heading>
            {loading ? (
                <Section>
