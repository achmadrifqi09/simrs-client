...
interface EducationLevelTableProps {
    ...
    selectRecord: React.Dispatch<React.SetStateAction<EducationDTO | null>>
...
const EducationLevelTable = (
    ...
    const url: string = '/master/education-level'
    const {status} = useSession();
    const [searchKeyword, setSearchKeyword] = useState<string>('');
    const {data, loading, error, getData} = useGet<EducationDTO[]>({
        url: url,
    ...
    return (
        <>
            ...
            <Table>
                ...
                <TableBody>
                    {
                        data?.map((educationLevel: EducationDTO, index: number) => {
                            return (
                                <React.Fragment key={index}>
                                    <TableRow>
                                        <TableCell className="font-medium">{index + 1}</TableCell>
                                        <TableCell className="font-medium">{educationLevel.nama_tingkat_pendidikan}</TableCell>
                                        <TableCell>
                                            {
                                                permission?.can_update ? (

                                                    <Switch
                                                        checked={educationLevel.status === 1}
                                                        onCheckedChange={
                                                            () => {
                                                                selectRecord(educationLevel);
                                                                setAction(Action.UPDATE_STATUS)
                                                            }
                                                        }
                                                    />
                                                ) : (educationLevel.status === 1 ? 'Aktif' : 'Non Aktif')
                                            }
                                        </TableCell>
                                        {
                                            (permission?.can_update || permission?.can_delete) && (
