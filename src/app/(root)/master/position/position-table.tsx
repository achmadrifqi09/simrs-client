...
interface PositionProps {
    ...
    selectRecord: React.Dispatch<React.SetStateAction<PositionDTO | null>>
...
const PositionTable = (
    ...
    const url: string = '/master/position'
    const {status} = useSession();
    const [searchKeyword, setSearchKeyword] = useState<string>('');
    const {data, loading, error, getData} = useGet<PositionDTO[]>({
        url: url,
    ...
    return (
        <>
            ...
            <Table>
                ...
                <TableBody>
                    {
                        data?.map((position: PositionDTO, index: number) => {
                            return (
                                <React.Fragment key={index}>
                                    <TableRow>
                                        <TableCell className="font-medium">{index + 1}</TableCell>
                                        <TableCell className="font-medium">{position.nama_jabatan}</TableCell>
                                        <TableCell>
                                            <Switch
                                                checked={position.status === 1}
                                                onCheckedChange={
                                                    () => {
