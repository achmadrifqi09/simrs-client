                            ...
                            <div className="flex gap-3 items-center justify-between">
                                <div className="flex gap-3 items-center">
                                    ...
                                    <Heading headingLevel="h4" variant="section-title" className="m-0">
                                        <span className="capitalize">{doctor?.gelar_depan?.toLocaleLowerCase()}. </span>
                                        <span className="capitalize">{doctor.nama_dokter.toLocaleLowerCase()} </span>
                                        <span className="capitalize">
                                            {doctor?.gelar_belakang?.toLocaleLowerCase()}
                                        </span>
                                    </Heading>
                                </div>
                            </div>
                                ...
                                {doctor.jadwal.map((schedule: QueueSchedule, index: number) => {
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            <div className="w-full flex flex-col justify-between h-full md:min-h-32 lg:min-h-28">
                                                ...
                                                <div>
                                                    ...
                                                    <div>
                                                        <p className="text-gray-500 mb-1">
                                                            Kuota pasien {schedule.kuota_terisi} /{' '}
                                                            {schedule.kuota_onsite}
                                                        </p>
                                                        <Progress
                                                            value={
                                                                (schedule.kuota_terisi / schedule.kuota_onsite) * 100
                                                            }
                                                            className="h-2.5"
                                                        />
                                                    </div>
                                                ...
                                                {index === 0 &&
                                                    doctor.jadwal[0]?.tanggal_libur &&
                                                    checkDateIsNow(doctor.jadwal[0]?.tanggal_libur) && (
                                                        <Badge className="w-min absolute top-6 right-4">Libur</Badge>
                                                    )}
                                                <div className="mt-4">
                                                    {!checkRegistrationAvailability(schedule) ||
                                                    (doctor.jadwal[0]?.tanggal_libur &&
                                                        checkDateIsNow(doctor.jadwal[0]?.tanggal_libur)) ? (
                                                        <Button size="sm" disabled={true} className="w-min">
                                                            Tutup
                                                        </Button>
                                                    ) : (
                                                        <Button
            ...
            <Dialog open={openSelectPatientType} onOpenChange={handleCloseDialog}>
                <DialogContent>
                    ...
                    <div className="mt-4">
                        ...
                        >
                            <Label className="flex items-center space-x-2 w-full px-3 py-4 border border-gray-300 rounded-md hover:cursor-pointer">
                    ...
                    {openSelectPatientType &&
                        selectedSchedule?.id_jadwal_dokter &&
                        (patientType ? (
                            <Button asChild>
                                <Link
