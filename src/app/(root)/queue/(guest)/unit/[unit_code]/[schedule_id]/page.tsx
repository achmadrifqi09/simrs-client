...
const Register = () => {
    ...
    useEffect(() => {
        if (
            isNaN(Number(searchParams.get('patient_type'))) ||
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden rounded-xl pb-6">
                ...
                <div>
                    {loading && !data ? (
                        ...
                        <div className="text-center">
                            <h4 className="text-center mb-0 text-xl font-semibold">
                                {data?.unit?.nama_unit_kerja} -{' '}
                                <span className="capitalize">{data?.pegawai?.gelar_depan?.toLocaleLowerCase()}. </span>
                                <span className="capitalize">{data?.pegawai?.nama_pegawai?.toLocaleLowerCase()} </span>
                                <span className="capitalize">{data?.pegawai?.gelar_belakang?.toLocaleLowerCase()}</span>
                            </h4>
                            <p className="text-sm text-gray-500">
                                {data?.jam_buka_praktek &&
                                    data?.jam_tutup_praktek &&
                                    `${timeStringFormatter(data?.jam_buka_praktek)} - ${timeStringFormatter(
                                        data?.jam_tutup_praktek
                                    )}`}
                            </p>
                        </div>
            ...
            <div className="px-6 py-6 overflow-auto custom-scroll">
                {loading && !data ? (
