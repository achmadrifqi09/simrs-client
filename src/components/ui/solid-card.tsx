...
const SolidCard = ({children, onClick, href = '#', type = 'link', disabled = false}: SolidCardProps) => {
        ...
        <>
            {
                type === 'button' ? (
                    <button
                        onClick={onClick}
                        disabled={disabled}
                        className={cn(baseClass, disabled ? 'opacity-80 cursor-not-allowed' : 'opacity-100')}>
